...
def get_user_by_email(email: str):
    ...
    return client[MONGO_DB][USERS_COLLECTION].find_one(
...
def get_app_url() -> str:
    try:
        ...
        return "https://projetcloudcomputing-smartstudy.streamlit.app"
...
def save_quiz_to_history(
    ...
    user_msg = (
        f"📝 **Quiz completed** — {len(questions)} questions on this document."
    )
    ...
    for i, q in enumerate(questions):
        ...
        if user_answer is not None:
            lines.append(
                f"- Your answer: {chr(65 + user_answer)}. {q['options'][user_answer]}"
            )
        if not is_correct and user_answer is not None:
            lines.append(
                f"- Correct answer: {chr(65 + correct)}. {q['options'][correct]}"
            )
...
if "reset_token" in query_params and not st.session_state.authenticated:
    ...
    if not email:
        ...
        new_pw2 = st.text_input(
            "Confirm password", type="password", key="reset_pw2"
        )

        if st.button(
            "Save new password", type="primary", use_container_width=True
        ):
...
def show_auth_page():
    ...
    with tab_login:
        ...
        if st.button(
            "Sign In", type="primary", use_container_width=True, key="btn_login"
        ):
        ...
        with st.expander("🔓 Forgot your password?"):
            forgot_email = st.text_input(
                "Your email", key="forgot_email", placeholder="you@example.com"
            )
            if st.button(
                "Send reset link", use_container_width=True, key="btn_forgot"
            ):
                if not forgot_email:
    ...
    with tab_signup:
        ...
        new_username = st.text_input(
            "Name / Username", key="signup_username", placeholder="e.g. Marie"
        )
    ...
    with tab_guest:
        ...
        st.info(
            "In guest mode, your conversations are not saved. They disappear when you close the tab."
        )
        if st.button(
            "Continue as Guest", use_container_width=True, key="btn_guest"
        ):
            st.session_state.authenticated = True
...
with st.sidebar:
    ...
    mode = st.radio(
        ...
        format_func=lambda x: (
            "🎓 Persona Mentor" if x == "persona" else "📝 Direct Mode"
        ),
    )
...
if st.session_state.file_ready and st.session_state.show_quiz:
    ...
    if st.session_state.quiz_data:
        ...
        if not st.session_state.quiz_submitted:
            st.info(
                f"📋 **{len(questions)} questions** — Choose one answer per question, then submit."
            )
            for i, q in enumerate(questions):
            ...
            if not all_answered:
                st.caption(
                    f"Answered: {len(st.session_state.quiz_answers)}/{len(questions)}"
                )
            ...
            for i, q in enumerate(questions):
                ...
                with st.container(border=True):
                    ...
                    for j, opt in enumerate(q["options"]):
                        ...
                        if j == correct:
                            st.markdown(
                                f"- **{prefix}. {opt}** *(correct answer)*"
                            )
                        elif j == user_answer and not is_correct:
                            st.markdown(
                                f"- {prefix}. {opt} *(your answer)*"
                            )
                        else:
