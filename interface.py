...
def save_quiz_to_history(session_id: str, questions: list, answers: dict, score: int, total: int):
    """Sauvegarde le résultat du quiz dans MongoDB comme un message spécial."""
    pct = round(100 * score / total)

    # Message utilisateur : indique qu'un quiz a été fait
    user_msg = f"📝 **Quiz effectué** — {len(questions)} questions sur ce document."

    # Message assistant : résumé complet du quiz
    ...
    for i, q in enumerate(questions):
        ...
        lines.append(f"- Ta réponse : {chr(65 + user_answer)}. {q['options'][user_answer]}" if user_answer is not None else "- Ta réponse : —")
        if not is_correct:
...
with st.sidebar:
    ...
    if st.button("✏️ Nouvelle conversation", use_container_width=True):
        for key in ["file_ready", "messages", "current_filename", "session_id",
                    "quiz_data", "quiz_answers", "quiz_submitted", "show_quiz"]:
...
if st.session_state.file_ready and st.session_state.show_quiz:
    ...
    if st.session_state.quiz_data:
        ...
        if not st.session_state.quiz_submitted:
            ...
            # Sauvegarde automatique du quiz dans MongoDB
