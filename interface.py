...
def load_session_messages(session_id: str):
    """Load messages from MongoDB for a given session_id."""
    try:
        client = MongoClient(MONGO_URI)
        col = client[MONGO_DB][MONGO_COLLECTION]
        ...
        messages = []
        for doc in docs:
            # langchain_mongodb stocke les messages dans le champ "History"
            # qui peut être une string JSON ou un dict
            ...
            # Le contenu peut être dans data.content ou directement dans content
    ...
    if not past_sessions:
        ...
        for s in past_sessions:
            ...
            if st.button(label, key=f"sess_{sid}", use_container_width=True):
                ...
                # Restore full session state so the user can continue chatting
...
# --- SECTION 2B : CHAT ---
    ...
    # Affiche tous les messages de la conversation (ancienne ou nouvelle)
            ...
            with st.spinner("Je réfléchis..."):
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        # Sauvegarde dans MongoDB avec le même session_id
                        # (que ce soit une nouvelle conv ou une ancienne reprise)
